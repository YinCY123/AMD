...
# gene of interest   
```{r}
goi <- readRDS(file = "/home/yincy/git/R-codes/Retina/amd-related-genes/amd_related_genes.rds") %>% 
  append(readRDS(file = "/home/yincy/git/R-codes/Retina/amd-related-genes/amd_genes_in_cellpress.rds")) %>% 
  append(readRDS(file = "/home/yincy/git/R-codes/Retina/amd-related-genes/ng_table5.rds")) %>% 
  unique()
...
saveRDS(object = ligand_activities_heatmap, 
        file = "/home/yincy/git/R-codes/Retina/res/ligand_activity_heatmap.rds")
```
...
saveRDS(object = ligand_target_heatmap, 
        file = "/home/yincy/git/R-codes/Retina/res/ligand_target_heatmap.rds")
```
...
saveRDS(object = expression_ligands_ridges, 
        file = "/home/yincy/git/R-codes/Retina/res/expression_ligands_ridges.rds")
```
...
saveRDS(object = statistic_significance_targets, 
        file = "/home/yincy/git/R-codes/Retina/res/statistic_significance_targets.rds")
```
...
saveRDS(object = expression_of_targets_boxplot, 
        file = "/home/yincy/git/R-codes/Retina/res/expression_targets_RPE_boxplot.rds")
```

# expression of regulator  
```{r}
y_filtered <- readRDS(file = "/home/yincy/git/R-codes/Retina/res/GSE99248_y_filtered.rds")
expression_regulators <- y_filtered %>% edgeR::cpm() %>% .[y_filtered$genes$geneSymbol %in% c("RUNX2", "AR", "ESR1", "SMAD3"), ] %>% 
  ...
  gather(key = "sample", value = "expression", -geneSymbol) %>% 
  mutate(group = ifelse(sample %in% early_amd_samples, "Early AMD", "Normal"), 
         geneSymbol = factor(geneSymbol, levels = c("RUNX2", "AR", "ESR1"))) %>% 
  ggplot(aes(reorder(geneSymbol, expression), expression)) +
  geom_boxplot(aes(color = group)) + 
  geom_jitter(aes(color = group), show.legend = F, width = 0.1) +
  scale_x_discrete(name = NULL) +
...
statistic_significance_regulator <- fit_cont %>% 
  as.data.frame() %>% 
  arrange(p.value) %>% 
  filter(genes.geneSymbol %in% c("RUNX2", "ESR1")) %>% 
  ggplot(aes(reorder(genes.geneSymbol, log10(p.value)), -log10(F.p.value))) +
...
saveRDS(object = statistic_significance_regulator, file = "/home/yincy/git/R-codes/Retina/res/statistic_significance_regulator.rds")
```
...
```{r}
png(filename = "/home/yincy/git/R-codes/Retina/res/TGFB1-COL10A1-signaling-network.png")
...
write.csv(x = data_source_net_TGFB1_COL10A1, 
          file = "/home/yincy/git/R-codes/Retina/res/supporting_datasource_TGFB1_COL10A1.csv")
```
